...
function getDrittel(score: number) {
  if (score >= 51) return {
    ...
    color: '#f97316',
    barWidth: 55,
    text: 'Gut aufgestellt. Noch einzelne Einfallstore schließen und deine Kunden sind sicher.',
  }
  return {
    label: 'Unteres Drittel',
    color: '#ef4444',
    barWidth: 25,
    text: 'Über 70% deiner Branche sind besser geschützt. Jetzt handeln.',
  }
}

export default function BranchenRanking({ score, industry }: Props) {
  ...
  return (
    <div className="rounded-2xl p-6 bg-white/5 border border-white/10 backdrop-blur-sm">
      <div className="flex items-center gap-2 mb-3">
        <Trophy size={18} style={{ color: '#FF2D55' }} />
      ...
      <div className="w-full bg-white/10 rounded-full h-3 mb-3">
        <div
          ...
          style={{ width: `${drittel.barWidth}%`, backgroundColor: '#FF2D55' }}
        />
      </div>
