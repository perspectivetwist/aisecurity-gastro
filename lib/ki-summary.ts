...
export async function generateKiSummary(
  ...
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Restaurant: ${restaurantName} | Befunde: ${sanitizeInput(befunde)} | JSON: { "zusammenfassung": "3-4 Sätze: Welche unkontrollierten Risiken durch KI-Präsenz entstehen — heute, ohne Fix." }`,
      },
    ],
  })
